...
(() => {
  ...
  function renderQuestionReview() {
    ...
    const answerKey = `<section class="answer-key-panel"><div class="answer-key-panel__head"><div><p class="section-label">Dokumen internal guru</p><h2>Kunci jawaban</h2><p>Kunci dipisahkan dari daftar soal siswa. Periksa dan ubah jawaban dari panel ini.</p></div><span class="badge badge--yellow">Jangan dibagikan</span></div><div class="answer-key-grid">${state.questions.map((question, index) => `<label><span>${String(index + 1).padStart(2, '0')} · ${escapeHtml(question.type)}</span>${question.type === 'Pilihan Ganda' ? `<select data-q-field="correct" data-q-id="${question.id}">${question.options.map((_, optionIndex) => { const letter = String.fromCharCode(65 + optionIndex); return `<option ${question.correct === letter ? 'selected' : ''}>${letter}</option>`; }).join('')}</select>` : `<input data-q-field="correct" data-q-id="${question.id}" value="${escapeHtml(question.correct)}" />`}</label>`).join('')}</div></section>`;
    return `<div class="questions-review"><div class="review-banner"><span class="review-banner__icon">✦</span><div><strong>${state.questions.length} contoh soal siap ditinjau dari ${total} soal.</strong><p>Daftar soal tidak menampilkan kunci. Gunakan panel kunci internal di bawah untuk memeriksa jawaban.</p></div></div><div class="editor-toolbar"><div class="editor-toolbar__title"><strong>${escapeHtml(state.questionConfig.subject)} · ${escapeHtml(state.questionConfig.topic)}</strong><small>${total} soal · ${escapeHtml(state.questionConfig.grade)}</small></div><div class="editor-toolbar__actions"><button class="mini-button" type="button" data-action="question-add">Tambah manual</button><button class="mini-button mini-button--primary" type="button" data-action="question-more">Generate soal tambahan</button></div></div><div class="questions-list" id="questions-list">${state.questions.map((question, index) => renderQuestionCard(question, index)).join('')}</div>${answerKey}<div class="builder-summary"><div class="builder-summary__numbers"><span><strong>${total}</strong> soal</span><span><strong>${state.questionConfig.multiple}</strong> pilihan ganda</span><span><strong>${state.questionConfig.short}</strong> isian</span><span><strong>${state.questionConfig.essay}</strong> essay</span><span>Perkiraan <strong>${escapeHtml(state.questionConfig.duration)}</strong></span></div><div class="button-row"><button class="button button--quiet" type="button" data-action="question-save">Simpan draft</button><button class="button button--primary" type="button" data-action="question-publish">Terbitkan ulangan <span data-icon="arrow" aria-hidden="true"></span></button></div></div></div>`;
  }

  function renderQuestionCard(question, index) {
    const optionHtml = question.type === 'Pilihan Ganda' ? `<div class="option-list">${question.options.map((option, optionIndex) => { const letter = String.fromCharCode(65 + optionIndex); return `<label class="question-option"><b>${letter}</b><input class="question-option__input" data-q-field="option-${optionIndex}" data-q-id="${question.id}" value="${escapeHtml(option)}" aria-label="Pilihan ${letter}" /></label>`; }).join('')}</div>` : '';
